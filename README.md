...
## Project Overview
Bambu Lab P2S 3D 프린터의 출력물을 자동으로 제거하고 다음 작업을 시작하는 로봇팔 자동화 시스템. 2026년 최신 로보틱스 트런드인 **Sim-to-Real (Isaac Lab)** 및 **VLA (Vision-Language-Action)** 아키텍처를 기반으로 설계됨.

### Core Objectives
- **End-to-End Automation:** P2S 출력 완료 감지 → 도어 개봉 → 출력물 수거 → 도어 폐쇄 → 다음 작업 시작.
...
- **Input:** RTSP/USB Camera Stream + P2S MQTT Status.
...
| **Printer** | Bambu Lab P2S | LAN Mode (MQTT/FTPS) enabled |

---

## Automation Cycle
1. **[IDLE]** P2S MQTT `FINISH` 신호 대기 및 베드 냉각 확인.
...
5. **[NEXT]** P2S MQTT로 다음 G-code 전송 및 출력 시작.
